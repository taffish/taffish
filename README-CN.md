...
| 资源 | 作用 |
| --- | --- |
| [taffish/taffish](https://github.com/taffish/taffish) | 当前仓库。`taf`、`taffish` 和 `taffish-mcp` 的二进制分发仓库。 |
...
当前 TAFFISH 是 `0.4.0`。运行时配置文件从 `0.2.0` 开始引入，用来稳定支持镜像源和自定义源。默认配置路径是：
...
--version VERSION         Release 版本 [0.4.0]
...
TAFFISH `0.4.0` 新增了 `taffish-mcp`，这是一个保守的 stdio MCP server，
面向 AI 客户端暴露 TAFFISH 能力。第一版 MCP 接口提供相对安全的 project、Hub、config、
