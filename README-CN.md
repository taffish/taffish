...
- `taffish`：编译并运行 `.taf` 程序。
- `taf`：管理 TAFFISH app 项目和本地 TAFFISH Hub 包。

当前仓库只分发预编译二进制文件。公开 release 包含安装脚本、shell 自动补全文件、Vim 语法高亮文件，以及各平台的二进制资产。
...
可以用 `--os` 和 `--arch` 强制选择平台，但对应二进制资产必须已经存在于 release 中。
...
--version VERSION         Release 版本 [0.1.2]
...
--share-url URL           覆盖 completion/vim archive URL
...
Tag archive 提供 `completion/` 和 `vim-highlight/` 文件。二进制 assets 提供 `taf` 和 `taffish`。
