...
Hub 系统位于 `taf-core/hub/`，负责 TAFFISH 应用的索引、查询、搜索、安装、维护、卸载和定位。
...
| `upgrade.lisp` | 计划和执行 outdated/install-all/upgrade/prune 操作。 |
...
1. index schema 是否变化。
2. 本地缓存路径是否变化。
3. 安装、卸载、list、which、outdated、upgrade、prune 是否保持一致。
