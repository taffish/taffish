...
| `repo` | string | `repository` 的兼容别名；消费者应把它规范化为 `repository`。 |
...
如果同时写了 `repository` 和 `repo`，它们必须具有相同值。新项目推荐使用
`repository`；已经使用 `repo` 的已有项目仍然有效。

`[upstream].license` 记录上游软件或资源自己的 license；它不同于
`[package].license`，后者记录 TAFFISH wrapper 项目本身的 license。两者可以不同。

对于学术型生信工具，推荐写入经过确认的引用信息。`citation` 用于简短的人类可读
引用文本；如果已知 DOI 和 PMID，应分别写入专门的 `doi` 和 `pmid` 字段。不要猜测
引用元数据。
